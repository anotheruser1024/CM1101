...
def print_inventory_items(items):
    ...
    #print("You have: " + "".join(str(items)) + ".")
    a = list_of_items(items)
...
def print_menu(exits, room_items, inv_items):
    ...
    for item in room_items:
        print("TAKE %s to take a %s" % (item["id"].upper(), item["name"]))

    for item in inv_items:
        print("DROP %s to drop your %s" % (item["id"].upper(), item["name"]))
...
def execute_go(direction):
    ...
    """
    global current_room
    e = is_valid_exit(current_room["exits"],direction)
    if e: 
