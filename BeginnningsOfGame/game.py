...
def remove_punct(text):
    ...
    for s in string.punctuation:
        t = t.replace(s,'') # The pass statement does nothing. Replace it with the body of your function.
    return t
...
def normalise_input(user_input):
    ...
    'help'
    """
    a = remove_spaces(remove_punct(user_input))
    return a.lower()  
...
def print_menu(exits):
    ...
    for i in exits:
        print("GO %s to %s" % (i.upper(), exits[i]))
...
def is_valid_exit(exits, user_input):
    ...
    True
    """
    return True if user_input in exits else False
...
def menu(exits):
    ...
    while True:
        ...
        # Normalise the input
        a = normalise_input(player_choice)
        # Check if the input makes sense (is valid exit)
            # If so, return the player's choice
        if is_valid_exit(exits, a) == True:
            return a
        elif is_valid_exit(exits, a) == False:
            print( player_choice + " isn't an option now")
            time.sleep(1)

        
        
       # if is_valid_exit() == True: break 
...
def move(exits, direction):
    ...
    False
    """
    if direction in exits:  True
    


    for i in exits:
      if direction == i:
        return rooms[exits[i]]    
